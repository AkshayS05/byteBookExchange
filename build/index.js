...
class Like {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(bbeData.root_url + "/wp-json/bbe/v1/manageLike", {
...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](bbeData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(bbeData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log("Sorry");
    }
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(bbeData.root_url + "/wp-json/bbe/v1/search?term=" + this.searchField.value);
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          ...
          <div class="one-third">
            ...
            ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No programs match that search. <a href="${bbeData.root_url}/programs">View all programs</a></p>`}
          ...
          <div class="one-third">
            ...
            ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>No campuses match that search. <a href="${bbeData.root_url}/campuses">View all campuses</a></p>`}
